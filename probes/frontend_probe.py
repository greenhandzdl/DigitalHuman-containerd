...
外壳 → backend → adapter → Fay → 对话端点（`.env` 里那个，本机是栈外的 26B），
看这一串回不回得来。

只发一问。这一问的耗时上限看对话端点：那组 172~301s 量自对话还压在宿主 ollama、9b 只剩
6% 权重驻留显存的几轮；端点搬到栈外那台 26B 之后同一问实测 5.6~18.2s，但仍有过 90.2s
撞在外壳那 90s 上的记录（2026-09-23）。后端侧的等待上限是 520s，默认 --timeout 600
就是照最坏那条链给的余量。会话复用那条判据
